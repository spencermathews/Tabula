...
class Wedges extends TSPattern {
  ...
  public void run(double deltaMs) {
    ...
    for (LED led : model.leds) {
      colors[led.index] = LXColor.hsb(
        Math.floor((rotation - led.transformedTheta) / quant) * quant + vHue * 360.0f,
        (1 - vSat) * 100,
        100);
    }     
  } 
  ...
  public void run(double deltaMs) {
    ...
    for (LED led : model.leds) {
      ...
      for (ColorBar colorBar : colorBars) {
        if (colorBar.intersects(bouncedNow, led.transformedY)) {
          colors[led.index] = colorBar.getColor(pHue.getValuef() * 360);
          break;
        }
      }
    }     
