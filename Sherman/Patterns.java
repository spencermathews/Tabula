...
class DoubleHelix extends TSPattern {
  ...
  public void run(double deltaMs) {
    ...
    for (LED led : model.leds) {
      float coilf = coil.getValuef() * (led.cy - model.cy);
      colors[led.index] = lx.hsb(
        lx.getBaseHuef() + .4f*Utils.abs(led.transformedY - model.cy) +.2f* Utils.abs(led.transformedTheta - 180),
        100,
        Utils.max(0, 100 - 2*LXUtils.wrapdistf(led.transformedTheta, theta.getValuef() + coilf, 180))
      );
    }
  }
}
...
class ColoredLeaves extends TSPattern {
  ...
  public void run(double deltaMs) {
    ...
    for (LED led : model.leds) {
      colors[led.index] = lx.hsb(
        (360 + movement[led.index  % movement.length].getValuef()) % 360,
        100,
        bright[led.index % bright.length].getValuef()
      );
    }
  }
}
...
class Twister extends TSPattern {
  ...
  public void run(double deltaMs) {
    ...
    for (LED led : model.leds) {
      float wrapdist = LXUtils.wrapdistf(led.transformedTheta, spinf + (model.yMax - led.transformedY)*coilf, 360);
      float yn = (led.transformedY / model.yMax);
      float width = 10 + 30 * yn;
      float df = Utils.max(0, 100 - (100 / 45) * Utils.max(0, wrapdist-width));
      colors[led.index] = lx.hsb(
        (lx.getBaseHuef() + .2f*led.transformedY - 360 - wrapdist) % 360,
...
class SweepPattern extends TSPattern {
  ...
  public void run(double deltaMs) {
    ...
    for (LED led : model.leds) {
      float yp = yPos.getValuef() + amp.getValuef() * Utils.sin((led.cx - model.cx) * .01f + offset.getValuef());
      colors[led.index] = lx.hsb(
        (lx.getBaseHuef() + Utils.abs(led.x - model.cx) * .2f +  led.cz*.1f + led.cy*.1f) % 360,
        Utils.constrain(Utils.abs(led.transformedY - model.cy), 0, 100),
        Utils.max(0, 100 - (100/width.getValuef())*Utils.abs(led.cy - yp - height.getValuef()))
      );
    }
  }
}
