...
class SparkleHelix extends TSPattern {
  ...
  SparkleHelix(LX lx) {
    ...
    sparkleTimeOuts = new int[model.leds.size()];
  }
  ...
  public void run(double deltaMs) {
    ...
    for (LED led : model.leds) {
      ...
      float spiralVal = Utils.max(0, 100 - (100*Utils.TWO_PI / (compensatedWidth))*LXUtils.wrapdistf((Utils.TWO_PI / 360) * led.transformedTheta, 8*Utils.TWO_PI + spin.getValuef() + coil.getValuef()*(led.transformedY-model.cy), Utils.TWO_PI));
      float counterSpiralVal = counterSpiralStrength.getValuef() * Utils.max(0, 100 - (100*Utils.TWO_PI / (compensatedWidth))*LXUtils.wrapdistf((Utils.TWO_PI / 360) * led.transformedTheta, 8*Utils.TWO_PI - spin.getValuef() - coil.getValuef()*(led.transformedY-model.cy), Utils.TWO_PI));
      float hueVal = (lx.getBaseHuef() + .1f*led.transformedY) % 360;
      if (sparkleTimeOuts[led.index] > Utils.millis()){        
        colors[led.index] = lx.hsb(hueVal, sparkleSaturation.getValuef(), 100);
      }
      else{
        colors[led.index] = lx.hsb(hueVal, 100, Utils.max(spiralVal, counterSpiralVal));        
        if (Utils.random(Utils.max(spiralVal, counterSpiralVal)) > sparkle.getValuef()){
          sparkleTimeOuts[led.index] = Utils.millis() + 100;
        }
      }
    }
  }
...
class MultiSine extends TSPattern {
  ...
  public void run(double deltaMs) {
    ...
    for (LED led : model.leds) {
      ...
      for (int i = 0; i < numLayers; i++){
        combinedDistanceSines[0] += Utils.sin(Utils.TWO_PI * frequencies[i].getValuef() + led.transformedY / distLayerDivisors[0][i]) / numLayers;
        combinedDistanceSines[1] += Utils.sin(Utils.TWO_PI * frequencies[i].getValuef() + Utils.TWO_PI*(led.transformedTheta / distLayerDivisors[1][i])) / numLayers;
      }
      ...
      colors[led.index] = lx.hsb(hueVal,  satVal, brightVal);
    }
  }
}
...
class Stripes extends TSPattern {
  ...
  public void run(double deltaMs) {
    ...
    for (LED led : model.leds) {  
      float hueVal = (lx.getBaseHuef() + .1f*led.transformedY) % 360;
      float brightVal = 50 + 50 * Utils.sin(spacing.getValuef() * (Utils.sin((Utils.TWO_PI / 360) * 4 * led.transformedTheta) + slopeFactor.getValuef() * led.transformedY)); 
...
class SparkleTakeOver extends TSPattern {
  ...
  public void run(double deltaMs) {
    ...
    for (LED led : model.leds) {  
      float newHueVal = (lx.getBaseHuef() + complimentaryToggle * hueSeparation + hueVariation.getValuef() * led.transformedY) % 360;
      float oldHueVal = (lx.getBaseHuef() + lastComplimentaryToggle * hueSeparation + hueVariation.getValuef() * led.transformedY) % 360;
      if (sparkleTimeOuts[led.index] > Utils.millis()){        
        colors[led.index] = lx.hsb(newHueVal,  (30 + coverage.getValuef()) / 1.3f, newBrightVal);
      }
      else {
        colors[led.index] = lx.hsb(oldHueVal,  (140 - coverage.getValuef()) / 1.4f, oldBrightVal);
        float chance = Utils.random(Utils.abs(Utils.sin((Utils.TWO_PI / 360) * led.transformedTheta * 4) * 50) + Utils.abs(Utils.sin(Utils.TWO_PI * (led.transformedY / 9000))) * 50);
        if (chance > (100 - 100*(Utils.pow(coverage.getValuef()/100, 2)))){
          sparkleTimeOuts[led.index] = Utils.millis() + 50000;
        }
...
class Lightning extends TSTriggerablePattern {
  private LightningLine bolt = null;
  ...
  public void run(double deltaMs) {
    ...
    if (triggered) {
      if (bolt.isDead()) {
        if (firesOnBeat.isOn()) {
          if (lx.tempo.beat()) {
            randomCheckTimeOut = Utils.millis() + 100;
            bolt = makeBolt();
          }
        } else {
          if (randomCheckTimeOut < Utils.millis()){
            randomCheckTimeOut = Utils.millis() + 100;
      ...
      for (LED led : model.leds) {
        float hueVal = 300;
        float lightningFactor = bolt.getLightningFactor(led.transformedY, led.transformedTheta);
...
class IceCrystals extends TSPattern {
  ...
  public void run(double deltaMs) {
    ...
    crystal.doUpdate();
    for (LED led : model.leds) {
      float lineFactor = crystal.getLineFactor(led.transformedY, led.transformedTheta);
      if (lineFactor > 110) {
