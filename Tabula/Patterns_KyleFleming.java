  ...
  public void run(double deltaMs) {
    if (count >= numBits) {
      ...
      for (LED led : model.leds) {
        setColor(led.index, led.index == count ? LXColor.WHITE : LXColor.BLACK);
      }
    }
...
class BassSlam extends TSTriggerablePattern {
  ...
  public void run(double deltaMs) {
    ...
    if (progress() < flashTimePercent) {
      ...
      for (LED led : model.leds) {
        setColor(led.index, lx.hsb(patternHue, 100, LXUtils.constrainf(100 - 2 * Utils.abs(y - led.transformedY), 0, 100)));
      }
    }
  }
...
//  }
  ...
  public void run(double deltaMs) {
    ...
    if (shouldAutofade) {
      for (LED led : model.leds) {
        blendColor(led.index, lx.hsb(0, 0, 100 * Utils.max(0, (float)(1 - deltaMs / fadeTime))), LXColor.Blend.MULTIPLY);
      }
    } else {
      clearColors();
    }
...
abstract class MultiObject extends Layer {
  ...
  public void run(double deltaMs) {
    if (running) {
      ...
      if (running) {
        for (LED led : model.leds) {
          blendColor(led.index, getColorForCube(led), LXColor.Blend.LIGHTEST);
        }
      }
    }
  }
  ...
  public int getColorForCube(LED led) {
    return lx.hsb(hue, 100, getBrightnessForCube(led));
  }
  
  public float getBrightnessForCube(LED led) {
    Vec2D cubePointPrime = VecUtils.movePointToSamePlane(currentPoint, led.transformedCylinderPoint);
    ...
    if (localLastPoint != null) {
      while (localLastPoint.distanceToSquared(currentPoint) > 100) {
        ...
        if (isInsideBoundingBox(led, cubePointPrime, point)) {
          dist = Utils.min(dist, getDistanceFromGeometry(led, cubePointPrime, point));
        }
        localLastPoint = point;
      }
    }

    if (isInsideBoundingBox(led, cubePointPrime, currentPoint)) {
      dist = Utils.min(dist, getDistanceFromGeometry(led, cubePointPrime, currentPoint));
    }
    return 100 * Utils.min(Utils.max(1 - dist / thickness, 0), 1) * fadeIn * fadeOut;
  }

  public boolean isInsideBoundingBox(LED led, Vec2D cubePointPrime, Vec2D currentPoint) {
    return VecUtils.insideOfBoundingBox(currentPoint, cubePointPrime, thickness, thickness);
  }

  public float getDistanceFromGeometry(LED led, Vec2D cubePointPrime, Vec2D currentPoint) {
...
class Explosion extends MultiObject {
  ...
  public float getBrightnessForCube(LED led) {
    Vec2D cubePointPrime = VecUtils.movePointToSamePlane(origin, led.transformedCylinderPoint);
...
class RandomColorGlitch extends TSPattern {
  ...
  final int brokenCubeIndex = (int)Utils.random(model.leds.size());
...
class ClusterLineTest extends TSPattern {
  ...
  public void run(double deltaMs) {
    ...
    for (LED led : model.leds) {
      ...
      colors[led.index] = lx.hsb(135, 100, 100
...
class ScrambleEffect extends Effect {
  ...
  protected void run(double deltaMs) {
    for (int i = Utils.min(model.leds.size() - 1, getAmount()); i > 0; i--) {
      colors[model.leds.get(i).index] = colors[model.leds.get((i + offset) % model.leds.size()).index];
    }
  }
}
...
class AcidTripTextureEffect extends Effect {
  ...
  public void run(double deltaMs) {
    if (amount.getValue() > 0) {
      for (int i = 0; i < colors.length; i++) {
        int oldColor = colors[i];
        LED led = model.leds.get(i);
        float newHue = Utils.abs(model.cy - led.transformedY) + Utils.abs(model.cy - led.transformedTheta) + trails.getValuef() % 360;
        int newColor = lx.hsb(newHue, 100, 100);
        int blendedColor = LXColor.lerp(oldColor, newColor, amount.getValuef());
        colors[i] = lx.hsb(LXColor.h(blendedColor), LXColor.s(blendedColor), LXColor.b(oldColor));
      }
  ...
  public void run(double deltaMs) {
    if (amount.getValue() > 0) {
      ...
      for (int i = 0; i < colors.length; i++) {
        ...
        double adjustedX = led.x / scale;
        double adjustedY = led.y / scale;
        double adjustedZ = led.z / scale;
...
class CandyCloud extends TSPattern {
  ...
  public void run(double deltaMs) {
    ...
    time += deltaMs;
    for (LED led : model.leds) {
      double adjustedX = led.x / scale.getValue();
      double adjustedY = led.y / scale.getValue();
      double adjustedZ = led.z / scale.getValue();
      ...
      float brightness = Utils.min(Utils.max((float)SimplexNoise.noise(led.x / 250, led.y / 250, led.z / 250 + 10000, time / 5000) * 8 + 8 - darkness.getValuef(), 0), 1) * 100;
      
      colors[led.index] = lx.hsb(hue, 100, brightness);
    }
  }
}
...
class GalaxyCloud extends TSPattern {
  ...
  public void run(double deltaMs) {
    ...
    time += deltaMs;
    for (LED led : model.leds) {
      float adjustedTheta = led.transformedTheta / 360;
      float adjustedY = (led.transformedY - model.yMin) / (model.yMax - model.yMin);
