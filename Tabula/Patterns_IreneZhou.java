...
class Fireflies extends TSTriggerablePattern {
  ...
  public void run(double deltaMs) {
    ...
    for (Firefly fly:fireflies) {
      for (LED led : model.leds) {
        if (Utils.abs(fly.yPos - led.transformedY) <= radius && Utils.abs(fly.theta - led.transformedTheta) <= radius) {
          float distSq = Utils.pow((LXUtils.wrapdistf(fly.theta, led.transformedTheta, 360)), 2) + Utils.pow(fly.yPos - led.transformedY, 2);
          float brt = Utils.max(0, 100 - Utils.sqrt(distSq * 4) - blinkers[fly.blinkIndex].getValuef());
          if (brt > LXColor.b(colors[led.index])) {
            colors[led.index] = lx.hsb(
...
class Lattice extends TSPattern {
  ...
  public void run(double deltaMs) {
    ...
    for (LED led : model.leds) {
      float wrapdistleft = LXUtils.wrapdistf(led.transformedTheta, spinf + (model.yMax - led.transformedY) * coilf, 180);
      float wrapdistright = LXUtils.wrapdistf(led.transformedTheta, -spinf - (model.yMax - led.transformedY) * coilf, 180);
      float width = yClimb.getValuef() + ((led.transformedY - yHeight.getValuef())/model.yMax) * 50;
      float df = Utils.min(100, 3 * Utils.max(0, wrapdistleft - width) + 3 * Utils.max(0, wrapdistright - width));

      colors[led.index] = lx.hsb(
        (hue.getValuef() + lx.getBaseHuef() + .2f*led.transformedY - 360) % 360, 
        100, 
        df
      );
    }
...
class Fire extends TSTriggerablePattern {
  ...
  public void run(double deltaMs) {
    ...
    for (LED led : model.leds) {
      float yn = (led.transformedY - model.yMin) / model.yMax;
      float cBrt = 0;
      float cHue = 0;
      float flameWidth = flameSize.getValuef() / 2;
      for (int i = 0; i < flames.size(); ++i) {
        if (Utils.abs(flames.get(i).theta - led.transformedTheta) < (flameWidth * (1- yn))) {
          cBrt = Utils.min(100, Utils.max(0, Utils.max(cBrt, (100 - 2 * Utils.abs(led.transformedY - flames.get(i).decay.getValuef()) - flames.get(i).decay.getBasisf() * 25) * Utils.min(1, 2 * (1 - flames.get(i).decay.getBasisf())) )));
...
class Bubbles extends TSTriggerablePattern {
  ...
  public void run(double deltaMs) {
    ...
    for (Bubble bubble: bubbles) {
      for (LED led : model.leds) {
        if (Utils.abs(bubble.theta - led.transformedTheta) < bubble.radius && Utils.abs(bubble.yPos - (led.transformedY - model.yMin)) < bubble.radius) {
          float distTheta = LXUtils.wrapdistf(bubble.theta, led.transformedTheta, 360) * 0.8f;
          float distY = bubble.yPos - (led.transformedY - model.yMin);
          ...
          if (distSq < bubble.radius * bubble.radius) {
            ...
            colors[led.index] = lx.hsb(
              ...
              Utils.constrain(led.transformedY/model.yMax * 125 - 50 * (dist/bubble.radius), 0, 100)
            );
          }
        }
      }
...
class Voronoi extends TSPattern {
  ...
  public void run(double deltaMs) {
    ...
    for (LED led : model.leds) {
      ...
      for (int i = 0; i < sites.length; ++i) {
        if (Utils.abs(sites[i].yPos - led.transformedY) < 150) { //restraint on calculation
          float distSq = Utils.pow((LXUtils.wrapdistf(sites[i].theta, led.transformedTheta, 360)), 2) + Utils.pow(sites[i].yPos - led.transformedY, 2);
...
class Pulley extends TSTriggerablePattern { //ported from SugarCubes
  ...
  public void run(double deltaMS) {
    ...
    if(triggered) {
      ...
      for (LED led : model.leds) {
        int gi = (int) Utils.constrain((led.x - model.xMin) * NUM_DIVISIONS / (model.xMax - model.xMin), 0, NUM_DIVISIONS-1);
        float yn =  led.transformedY/model.yMax;
        colors[led.index] = lx.hsb(
          (lx.getBaseHuef() + Utils.abs(led.x - model.cx)*.8f + led.transformedY*.4f) % 360, 
          Utils.constrain(100 *(0.8f -  yn * yn), 0, 100), 
          Utils.max(0, 100 - Utils.abs(led.transformedY/2 - 50 - gravity[gi].getValuef())*falloff)
        );
      }
    }
  }
...
class Springs extends TSPattern {
  ...
  public void run(double deltaMS) {
    ...
    for (LED led : model.leds) {
      float yn =  led.transformedY/model.yMax;
      float width = (1-yn) * 25;
      float wrapdist = LXUtils.wrapdistf(led.transformedTheta, spinf + (led.transformedY) * 1/(gravity.getValuef() + 0.2f), 360);
  ...
  public void run(double deltaMS) {
    ...
    if(triggered) {
      ...
      for (LED led : model.leds) {
        ...
        float cHue = 0;
        for (int j = 0; j < pulleys.size(); ++j) {
          cHue = (lx.getBaseHuef() + Utils.abs(led.x - model.cx)*.8f + led.transformedY*.4f + pulleys.get(j).baseHue) % 360;
          cBrt += Utils.max(0, pulleys.get(j).maxBrt.getValuef() * (100 - Utils.abs(led.transformedY/2 - 50 - pulleys.get(j).gravity.getValuef())*falloff));
        }
        float yn =  led.transformedY/model.yMax;
        colors[led.index] = lx.hsb(
          cHue, 
...
//    for (LED led : model.leds) {
//      led.transformedY = led.transformedY * ( 1 - ripple.getValuef() * amplitude.getValuef() * Utils.sin((led.transformedTheta + rotate) / 30 * Utils.PI ));
//    }
//  }
//}
...
//     for (LED led : model.leds) {
//       float distVal = Utils.sqrt(Utils.pow((LXUtils.wrapdistf(thetaCenter, led.transformedTheta, 360)) * 0.8f, 2) + Utils.pow(yCenter - led.transformedY, 2));
//       float heightHueVariance = 0.1f * led.transformedY;
//       if (distVal < radius){
...
//       }
//       colors[led.index] = lx.hsb(hueVal,  100, brightVal);
//     }
//   }
// }
