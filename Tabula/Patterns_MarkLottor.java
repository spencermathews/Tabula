...
class MarkLottor extends TSPattern {
  ...
  public void run(double deltaMs) {
    ...
    // dim everything already on cube
      for (LED led : model.leds) {
        colors[led.index] = lx.hsb(
          LXColor.h(colors[led.index]),
          LXColor.s(colors[led.index]),
          LXColor.b(colors[led.index]) * (1.0f-(p4v*p4v)));  
      }
      /*
    // dim everything already on cube
      for (LED led : model.leds) {
        colors[led.index] = lx.hsb(0,0,0);
      }
      */
    ...
    for (n = 0; n < maxballs; n++)
    {
      ...
      for (LED led : model.leds) {
	if ((Utils.abs(theta - led.transformedTheta) < (50*p1v)) &&
	    (Utils.abs(y - led.transformedY) < (50*p1v)))
	  colors[led.index] = balls[n].getcolor();
      }
    }
    ...
    for (Tree tree : model.trees) {
      ...
      for (LED led : model.leds) {
        ...
        colors[led.index] = lx.hsb(
          (lx.getBaseHuef() + led.transformedY * .3f) % 360,
          100,
          Utils.max(0, 100 - LXUtils.wrapdistf(led.transformedTheta, apf, 360))
        );
      }
    }
    */
