...
class Twinkle extends TSPattern {
  ...
  Twinkle(LX lx) {
    ...
    cubeToModulatorMapping = new int[model.leds.size()];
    ...
    for (LED led : model.leds) {
      if (sparkleTimeOuts[led.index] < Utils.millis()) {
        ...
        if (Utils.random(10) <= 3) {
          cubeToModulatorMapping[led.index] = (int)Utils.random(numBrights);
        }
        sparkleTimeOuts[led.index] = Utils.millis() + (int)Utils.random(11100, 23300);
      }
      colors[led.index] = lx.hsb(
      0, 
      0, 
      bright[cubeToModulatorMapping[led.index]].getValuef() * brightnessParam.getValuef()
        );
    }
  }
}
...
class VerticalSweep extends TSPattern {
  ...
  public void run(double deltaMs) {
    ...
    for (LED led : model.leds) {
      float progress = ((led.transformedTheta / 360.0f) + range.getValuef()) % 1; // value is 0-1
      ...
      colors[led.index] = LXColor.lerp(color1, color2, amt);
    }
  }
}
